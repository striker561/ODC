...
function beginSignStep(
  seq: SignSequenceState,
  states: SignFingerState[],
  index: number,
): void {
  seq.index = index;
 ...
 */
export function useHandSignSequence(
  active: boolean,
  onSignChange: ((sign: string | null) => void) | undefined,
) {
  const statesRef = useRef<SignFingerState[]>(
    Array.from({ length: 5 }, () => ({ progress: 0 })),
  );
  const seqRef = useRef<SignSequenceState>({
    index: 0,
    phase: "transition",
    stepElapsed: 0,
    fromProgress: [0, 0, 0, 0, 0],
  });
  const releaseRef = useRef<SignReleaseState>({
    active: false,
    elapsed: 0,
    from: [0, 0, 0, 0, 0],
  });
